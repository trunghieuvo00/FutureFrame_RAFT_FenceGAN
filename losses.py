...
class Intensity_Loss(nn.Module):
    ...
    def forward(self, gen_frames, gt_frames):
        return torch.mean(torch.abs((gen_frames - gt_frames) ** 2 / 2))
...
class Adversarial_Loss(nn.Module):
    ...
    def forward(self, fake_outputs, alpha = 0.5):
        ...
        return torch.mean((fake_outputs - alpha) ** 2 / 2)


class Discriminate_Loss_Fence(nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, real_outputs, fake_outputs, gamma = 0.1):
        return torch.mean(real_outputs** 2 / 2) + gamma*torch.mean((fake_outputs - 1) ** 2 / 2)
...
class Discriminate_Loss(nn.Module):
    ...
    def forward(self, real_outputs, fake_outputs):
        return 0.5*torch.mean((real_outputs - 1) ** 2 / 2) + torch.mean(fake_outputs ** 2 / 2)
