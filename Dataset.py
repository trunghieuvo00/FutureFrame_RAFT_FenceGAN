...
class Label_loader:
    def __init__(self, cfg, video_folders):
        assert cfg.dataset in ('ped1', 'ped2', 'avenue', 'shanghaitech'), f'Did not find the related gt for \'{cfg.dataset}\'.'
        self.cfg = cfg
    ...
    def load_shanghaitech(self):
        #np_list = glob.glob(f'{self.cfg.data_root + self.name}/testing/test_frame_masks/')
        np_list = glob.glob('/home/abnormal_detection/VoTrungHieu/ano_pred_cvpr2018_origin/Data/shanghaitech/testing/test_frame_mask/*.npy')
        np_list.sort()
