...
from models.R2U_Net.r2unet import R2U_Net
import statistics
...
def val(cfg, model=None):
    if model:  # This is for testing during training.
        ...
        generator = RRRRU_Net(img_ch=12, output_ch=3).cuda().eval()
    ...
    with torch.no_grad():
        for i, folder in enumerate(video_folders):
            ...
            print('Average fps', statistics.mean(avg_fps))
            fps_total_avg.append(statistics.mean(avg_fps))
            psnr_group.append(np.array(psnrs))  # psnrs is error frame level while psnr_group is error video level
    ...
    print('Total average fps: ', statistics.mean(fps_total_avg))
